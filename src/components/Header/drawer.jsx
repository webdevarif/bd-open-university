...
const Drawer = ({ list, isOpen }) => {
    ...
    <aside className={`header__drawer ${isOpen ? 'is-open' : ''}`}>
        ...
        {list && (
            ...
            {list.map((item, i) => (
                <li key={i}>
                <Link href={item.slug} className={`inline-block py-1 font-semibold text-md tracking-wide hover:text-primary-light border-s-2 hover:border-s-primary-light ps-3 transition-all duration-300 ${router.pathname === item.slug ? 'active text-primary-light border-s-primary-light' : ''}`}>
                    {item.name}
                </Link>
                </li>
            ))}
